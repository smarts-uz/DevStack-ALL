...
function ExpandPathToSelectNode(parentnode)
{
                                ...
                                URLPrefix, ChooseContainer, AvoidProtocolPrefix)
 {
     ...
     if (whichSide=="adddest")
        whichSide="right"; // for our purposes, we only need to know left or right
     ...
     var theURL="directorylisting.htm?path=" + encodeURIComponent(LDefaultPath) +
              "&token="+ GClientToken +
              "&isroot=true" +
              (GAddDestNum>=0 ? ("&adddestnum=" + GAddDestNum) : "") +
              "&side=" + whichSide +
              "&what=" + BrowseWhat;
     // alert("Invoking "+theURL);
     $.ajax({
